...
# Add project root to path - this file is in src/core/ so we need to go up two levels
...
try:
    ...
    try:
        ...
        # Final fallback - define a simple log function
...
class CycleStep(BaseTempStep):
    ...
    def expand_cycles(self):
        """Expand this cycle step into individual temperature steps."""
        if not self.is_cycle_step() or not self.cycle_count:
            return [self]
        
        expanded_steps = []
        
        for cycle_num in range(2, 2 + self.cycle_count):  # Cycles 2-22
            # Cold plateau step
            cold_step = DwellStep(
                step_name=f"CYCLE{cycle_num}_COLD_PLATEAU_MINUS34C",
                description=f"Cycle {cycle_num}: Cold plateau at -34°C with continuous monitoring",
                temperature=self.low_temp,
                target_temp_delta=self.target_temp_delta,
                temp_controller_offset=self.temp_controller_offset,
                psat_tests=False,
                gain_tests=False,
                phase_tests=False,
                S22_tests=False,
                S11_tests=False,
                IP3_tests=False,
                noise_figure_tests=False,
                pin_pout_tests=True,  # Continuous PIN-POUT monitoring
                na_tests=False,
                total_time=self.dwell_time_low // 60 if self.dwell_time_low else 60,  # Convert seconds to minutes
                actual_time=0,
                time_per_path=self.time_per_path,
                voltage=self.voltage,
                current=self.current,
                settlement_tolerance=self.settlement_tolerance,
                settlement_window=self.settlement_window,
                monitoring_interval=self.monitoring_interval,
                initial_delay=self.initial_delay,
                power_off_after=False
            )
            expanded_steps.append(cold_step)
            
            # Ramp to hot step
            ramp_hot_step = RampStep(
                step_name=f"CYCLE{cycle_num}_RAMP_TO_71C",
                description=f"Cycle {cycle_num}: Ramp from -34°C to 71°C at {self.ramp_rate}°C/min",
                temperature=self.high_temp,
                target_temp_delta=2.0,
                temp_controller_offset=getattr(self, 'temp_controller_offset_high', 1.0),
                ...
                IP3_tests=False,
                noise_figure_tests=False,
                pin_pout_tests=False,
                na_tests=False,
                total_time=int((self.high_temp - self.low_temp) / self.ramp_rate) if self.ramp_rate and self.high_temp and self.low_temp else 30,
                actual_time=0,
                time_per_path=0,
                voltage=self.voltage,
                current=self.current,
            ...
            expanded_steps.append(ramp_hot_step)
            
            # Hot plateau step
            hot_step = DwellStep(
                step_name=f"CYCLE{cycle_num}_HOT_PLATEAU_71C",
                description=f"Cycle {cycle_num}: Hot plateau at 71°C with continuous monitoring",
                temperature=self.high_temp,
                target_temp_delta=2.0,
                temp_controller_offset=getattr(self, 'temp_controller_offset_high', 1.0),
                psat_tests=False,
                ...
                pin_pout_tests=True,  # Continuous PIN-POUT monitoring
                na_tests=False,
                total_time=self.dwell_time_high // 60 if self.dwell_time_high else 60,  # Convert seconds to minutes
                actual_time=0,
                time_per_path=self.time_per_path,
                voltage=self.voltage,
                current=self.current,
            ...
            expanded_steps.append(hot_step)
            
            # Ramp back to cold (except for last cycle)
            if cycle_num < 2 + self.cycle_count - 1:
                ramp_cold_step = RampStep(
                    step_name=f"CYCLE{cycle_num}_RAMP_TO_MINUS34C",
                    description=f"Cycle {cycle_num}: Ramp from 71°C to -34°C at {self.ramp_rate}°C/min",
                    temperature=self.low_temp,
                    target_temp_delta=self.target_temp_delta,
                    temp_controller_offset=self.temp_controller_offset,
                    ramp_rate=self.ramp_rate,
                ...
                expanded_steps.append(ramp_cold_step)
