...
class TempStepsExecutor:
    ...
    def run_profile(
        ...
        # Session start
        logger.info("Steps cycle start | profile=%s | paths=%s", str(Path(profile_path).resolve()), use_paths)
        ...
        logger.info("Steps cycle end | status=ok")
    ...
    def _set_and_wait(
        ...
        self.temp_controller.set_setpoint(1, target)
        logger.info(
        ...
        while True:
            ...
            if actual is None:
                logger.info("TEMP_POLL | ch=1 | setpoint=%.2f | actual=None | retry_in=%ss", target, poll_s)
                time.sleep(poll_s)
                continue
            err = abs(actual - target)
            logger.debug("Actual=%.2f°C err=%.2f°C", actual, err)
            in_tol = err <= tolerance_c
            elapsed = time.time() - stable_start if stable_start is not None else 0.0
            logger.info(
                "TEMP_POLL | ch=1 | setpoint=%.2f | actual=%.2f | error=%.2f | in_tol=%s | elapsed_stable=%.1fs",
                target,
                actual,
                err,
                in_tol,
    ...
    def _execute_step(self, idx: int, step: BaseTempStep) -> None:
        ...
        if isinstance(step, RampStep):
            ramp_rate = getattr(step, "ramp_rate", None) or 1.0
            ...
            if dwell_minutes:
                ...
                logger.info("Dwell end | for %s minutes", dwell_minutes)
    ...
    def _run_tests_for_step(
        ...
        logger.info("Running tests: SIG_A=%s, NA=%s on %d paths", sig_a_tests, na_tests, len(paths))
        logger.info("Tests start | flags={sig_a=%s, na=%s, golden=%s}", sig_a_tests, na_tests, golden_tests)
        for p in paths:
            logger.info("Tests path | %s", p)
